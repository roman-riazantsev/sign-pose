...
x_1 = torch.autograd.Variable(torch.rand(4, 4), requires_grad=True)
x_2 = torch.autograd.Variable(torch.rand(4, 4), requires_grad=True)

stop_loss = 1e-3
step_size = stop_loss / 1000.0
print('Loss before: %s' % (torch.norm(torch.matmul(torch.matmul(A, x_1), x_2)[..., :-1] - b[..., :-1])))
print(b[..., :-1])
for i in range(100000000000000):
    Δ = torch.matmul(torch.matmul(A, x_1), x_2)[..., :-1] - b[..., :-1]
    L = torch.norm(Δ, p=2)
    L.backward()
    x_1.data -= step_size * x_1.grad.data  # step
    x_1.grad.data.zero_()

    x_2.data -= step_size * x_2.grad.data  # step
    x_2.grad.data.zero_()
    ...
    if abs(L) < stop_loss:
        ...
        break
    if i % 100000 == 0:
        print(f"x at step {i}: {x_1, x_2}. Loss: {abs(L)}")
print('Loss after: %s' % (torch.norm(torch.matmul(A, x_1)[..., :-1] - b[..., :-1])))
# print(x)
